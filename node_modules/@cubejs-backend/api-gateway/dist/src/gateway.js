...
const transformData = (aliasToMemberNameMap, annotation, data, query, queryType) => (data.map(r => {
    ...
    ramda_1.default.toPairs, ramda_1.default.map(p => {
        ...
        if (!annotationForMember) {
            throw new UserError_1.UserError(`You requested hidden member: '${p[0]}'. Please make it visible using \`shown: true\`. Please note primaryKey fields are \`shown: false\` by default: https://cube.dev/docs/schema/reference/joins#setting-a-primary-key.`);
        }
    ...
    initApp(app) {
        ...
        app.use(`${this.basePath}/graphql`, userMiddlewares, async (req, res) => {
            ...
            return express_graphql_1.graphqlHTTP({
                ...
                graphiql: shared_1.getEnv('nodeEnv') !== 'production'
            })(req, res);
        });
    ...
    async getPreAggregations({ cacheOnly, context, res }) {
        ...
        try {
            ...
            const preAggregationPartitions = await this.refreshScheduler()
                .preAggregationPartitions(context, compilerApi, query_1.normalizeQueryPreAggregations({
                timezones: this.scheduledRefreshTimeZones,
    ...
    async load({ query, context, res, ...props }) {
        ...
        try {
            ...
            this.log({
                ...
                queriesWithPreAggregations: results.filter((r) => Object.keys(r.usedPreAggregations || {}).length)
                    .length,
