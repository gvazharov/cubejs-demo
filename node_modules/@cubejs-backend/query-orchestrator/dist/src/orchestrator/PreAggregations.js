...
class PreAggregationLoader {
    ...
    async downloadTempExternalPreAggregation(client, newVersionEntry, preAggregation, saveCancelFn, queryOptions) {
        ...
        try {
            ...
            this.logger('Downloading external pre-aggregation error', {
                ...queryOptions,
                error: (error === null || error === void 0 ? void 0 : error.stack) || (error === null || error === void 0 ? void 0 : error.message)
            });
            throw error;
        }
    }
    async uploadExternalPreAggregation(tableData, newVersionEntry, saveCancelFn, queryOptions) {
        ...
        await saveCancelFn(externalDriver.uploadTableWithIndexes(table, tableData.types, tableData, this.prepareIndexesSql(newVersionEntry, queryOptions), this.preAggregation.uniqueKeyColumns, queryOptions)).catch((error) => {
            this.logger('Uploading external pre-aggregation error', { ...queryOptions, error: (error === null || error === void 0 ? void 0 : error.stack) || (error === null || error === void 0 ? void 0 : error.message) });
            throw error;
        });
...
class PreAggregations {
    ...
    async checkPartitionsBuildRangeCache(queryBody) {
        ...
        const result = await Promise.all(preAggregations.map(async (preAggregation) => {
            ...
            const isCached = preAggregation.partitionGranularity ? (await Promise.all(preAggregationStartEndQueries.map(([query, values]) => this.queryCache.resultFromCacheIfExists({ query, values })))).every((res) => res === null || res === void 0 ? void 0 : res.data) : true;
            return {
                preAggregation,
                isCached
            };
